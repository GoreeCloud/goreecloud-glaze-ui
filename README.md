...
`js/glaze-v1.4-accessibility-observation-capture.candidate.mjs` and the local reference surface at `reference/glaze-v1.4-accessibility-observation-capture.candidate.html` provide the next governed step without automating acceptance. The capture adapter accepts only exact-bound in-progress packets with pending human review, rejects duplicate scenario IDs or malformed support claims, and preserves the immutable source/tree identity. Local preference detection is exposed only as a reviewer hint: tested/unsupported preference states and pass/fail scenarios require explicit reviewer confirmation plus evidence references, and required or claimed assistive-technology scenarios cannot be marked not-applicable. Export is an explicit local JSON action. The adapter requires no network access, telemetry, analytics, persistent storage, browser-identity sniffing, or screen-capture APIs and always leaves human review pending with accessibility/lifecycle acceptance false.

Automated source-level evidence for the optical material, semantic runtime, composable accessibility runtime, Web renderer, browser capability, qualification-support, packet-preparation, and observation-capture slices is provided by five Python validators, six Python regression layers, `tests/glaze-v1.4-optical-runtime.test.mjs`, `tests/glaze-v1.4-accessibility-composition.test.mjs`, `tests/glaze-v1.4-optical-web.test.mjs`, `tests/glaze-v1.4-browser-capabilities.test.mjs`, `tests/glaze-v1.4-accessibility-observation-capture.test.mjs`, and the exact-head `Glaze V1.4 Optical Runtime Candidate` workflow. Exact implementation head `5c69c6428d9a8b1f07841240f0ebfba0d94440e9` passed workflow run `34665681174`, including all five V1.4 validators, all six Python regression layers, the blocked/non-promoting qualification-template check, an exact-source/tree-bound temporary packet smoke test, executable runtime/accessibility/Web/browser/observation-capture Node regressions, validator/evaluator/generator compilation, and the no-source-mutation check.
...
Source validation is governed by `scripts/validate_glaze_v1.4_optical_material.py`, `scripts/validate_glaze_v1_4_semantic_optical_runtime.py`, `scripts/validate_glaze_v1_4_browser_capabilities.py`, `scripts/validate_glaze_v1_4_accessibility_composition.py`, `scripts/validate_glaze_v1_4_accessibility_qualification.py`, `scripts/evaluate_glaze_v1_4_accessibility_qualification.py`, `scripts/prepare_glaze_v1_4_accessibility_qualification_packet.py`, their Python regressions, the runtime/accessibility/Web/browser/observation-capture Node regressions, and `.github/workflows/glaze-v1.4-optical-material.yml`.

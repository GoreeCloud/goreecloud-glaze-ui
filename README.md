...
The browser capability slice adds `contracts/v1.4/browser-capabilities.candidate.json` and `js/glaze-v1.4-browser-capabilities.candidate.mjs`. The adapter uses local, synchronous feature detection for bounded rendering capabilities and local media preferences. It fails closed when evidence is missing, never auto-declares environmental sampling, reflection, or HDR-aware luminance, and does not read browser identity, device-memory or hardware-concurrency values, battery/network details, screen dimensions, capture APIs, persistent storage, telemetry, or analytics. Browser-detected accessibility requirements are now preserved additively and independently; forced-colors is not collapsed into Increased Contrast for behavior, while the legacy recommended profile remains only a compatibility summary and is explicitly not qualification evidence.
...
Automated source-level evidence for the optical material, semantic runtime, composable accessibility runtime, Web renderer, and browser capability slices is provided by four Python validators/regression layers plus `tests/glaze-v1.4-optical-runtime.test.mjs`, `tests/glaze-v1.4-accessibility-composition.test.mjs`, `tests/glaze-v1.4-optical-web.test.mjs`, `tests/glaze-v1.4-browser-capabilities.test.mjs`, and the exact-head `Glaze V1.4 Optical Runtime Candidate` workflow. Exact implementation head `d6858b35b5c87ef8600580e3a6e68450bcdb0786` passed workflow run `34662883331`, including all four V1.4 validators, all four Python regression layers, executable runtime/accessibility/Web/browser Node regressions, validator compilation, and the no-source-mutation check.
...
Source validation is governed by `scripts/validate_glaze_v1.4_optical_material.py`, `scripts/validate_glaze_v1_4_semantic_optical_runtime.py`, `scripts/validate_glaze_v1_4_browser_capabilities.py`, `scripts/validate_glaze_v1_4_accessibility_composition.py`, their paired Python regressions, the runtime/accessibility/Web/browser Node regressions, and `.github/workflows/glaze-v1.4-optical-material.yml`.

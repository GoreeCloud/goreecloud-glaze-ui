#!/usr/bin/env python3
"""Validate the historical GLAZE UI V1.3 Expressive Shape workstream."""
...
def main() -> int:
    ...
    req(lifecycle.get("activeCandidate") is None, "Expressive Shape validation must not activate release lifecycle Candidate")
    ...
    req(contract.get("releaseLifecycle") == "proposed", "historical shape artifact lifecycle must remain Proposed")
    ...
    req(contract.get("consumerEligible") is False, "historical shape candidate artifact must not be consumer eligible")
    req(contract.get("sourceStable") == SOURCE_STABLE_VERSION, "historical shape artifact must preserve its V1.2 Stable source baseline")
    ...
    req(token.get("releaseLifecycle") == "proposed", "historical shape tokens must remain Proposed")
    req(token.get("lifecycleAuthority") is False, "shape tokens must not carry lifecycle authority")
    req(token.get("consumerEligible") is False, "historical shape tokens must not be consumer eligible")
    ...
    print("Boundary: historical V1.3 candidate artifacts preserve their V1.2 source provenance while repository lifecycle authority remains current V1.3.0 Stable; shape contracts are revalidated without new lifecycle or consumer claims.")
    return 0

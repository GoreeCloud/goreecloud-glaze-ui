#!/usr/bin/env python3
"""Validate the historical GLAZE UI V1.3 Motion and Continuity workstream."""
...
def main() -> int:
    ...
    req(lifecycle.get("activeCandidate") is None, "Motion validation must not activate lifecycle Candidate")
    ...
    req(contract.get("releaseLifecycle") == "proposed", "historical motion artifact lifecycle must remain Proposed")
    req(contract.get("lifecycleAuthority") is False, "motion contract must not carry lifecycle authority")
    req(contract.get("consumerEligible") is False, "historical motion candidate artifact must not be consumer eligible")
    req(contract.get("sourceStable") == SOURCE_STABLE_VERSION, "historical motion artifact must preserve its V1.2 Stable source baseline")
    ...
    req(token.get("releaseLifecycle") == "proposed", "historical motion tokens must remain Proposed")
    ...
    print("Boundary: historical V1.3 candidate artifacts preserve their V1.2 source provenance while repository lifecycle authority remains current V1.3.0 Stable; semantic connected motion and continuity are revalidated without new lifecycle or consumer claims.")
    return 0

...
def validate_source() -> None:
    ...
    require(contract.get("persistenceAndSync", {}).get("implementedByThisCandidate") is False, "Candidate overclaimed persistence/sync")
    ...
    require({"preference-persistence","cross-device-preference-sync","wallpaper-sampling","human-visual-review","native-platform-parity","stable","consumer-conformance"}.issubset(not_established), "personalization evidence boundary overclaims acceptance")
    ...
    for marker in ("Personalization changes expression", "Security review required", "No raw blur, saturation, or shadow sliders", "Follow System requires a platform adapter"):
...
def main() -> int:
    ...
    try:
        ...
        value = request("POST","/session",{"capabilities":{"alwaysMatch":{"browserName":"chrome","goog:chromeOptions":{"args":["--headless=new","--no-sandbox","--disable-dev-shm-usage","--disable-background-networking","--disable-component-update","--disable-extensions","--disable-sync","--metrics-recording-only","--no-first-run","--window-size=900,900"]}}}},timeout=60)
        ...
        execute(sid,"document.documentElement.dataset.glzAccent='ice';document.documentElement.dataset.glzAtmosphere='calm';return true;")
        ...
        execute(sid,"document.documentElement.dataset.glzAtmosphere='expressive';return true;")
        ...
        execute(sid,"document.documentElement.removeAttribute('data-glz-transparency');document.documentElement.dataset.glzDensity='comfortable';return true;")
        ...
        execute(sid,"document.documentElement.dataset.glzDensity='productive';return true;")
        ...
        execute(sid,"document.documentElement.dataset.glzAppearance='light';return true;")
        light = execute(sid,"return getComputedStyle(document.body).backgroundColor")
        execute(sid,"document.documentElement.dataset.glzAppearance='deep-dark';return true;")
        deep = execute(sid,"return getComputedStyle(document.body).backgroundColor")
        require(light != deep, "Light and Deep Dark appearance references did not differentiate")
        screenshot(sid,"deep-dark")

        cdp(sid,"Emulation.setDeviceMetricsOverride",{"width":320,"height":1100,"deviceScaleFactor":1,"mobile":False,"screenWidth":320,"screenHeight":1100})
        execute(sid,"document.documentElement.dataset.glzTextScale='200';document.documentElement.style.fontSize='200%';document.documentElement.dataset.glzDensity='productive';return true;")
        ...
        print("GLAZE UI V1.2 Personalization and Appearance rendered acceptance passed")
        return 0

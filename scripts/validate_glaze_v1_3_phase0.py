#!/usr/bin/env python3
"""Validate the Proposed GLAZE UI V1.3 Adaptive Resonance Phase 0 boundary."""
...
def main() -> int:
    ...
    if errors:
        print("GLAZE UI V1.3 Phase 0 validation FAILED:")
        for error in errors:
            print(f"- {error}")
        return 1
    ...
    req(
        ...
        "Phase 0 must leave VERSION at 1.2.0",
    )
    ...
    req(lifecycle.get("activeCandidate") is None, "Phase 0 must not activate a V1.3 Candidate")
    ...
    req(plan.get("phase") == "phase-0-foundation", "unexpected V1.3 phase")
    req(plan.get("phaseEffect") == "planning-readiness-only", "Phase 0 must remain planning-only")
    ...
    req(protected.get("phase0MayChangeLifecycleRegistry") is False, "Phase 0 must not authorize lifecycle promotion")
    req(protected.get("phase0MayCreateCandidateEntrypoints") is False, "Phase 0 must not authorize Candidate entrypoints")
    ...
    req(ids == expected_workstreams, "V1.3 Phase 0 workstream set is incomplete or unexpected")
    req(all(item.get("status") == "planned" for item in workstreams), "all Phase 0 workstreams must remain planned")
    ...
    req(not (ROOT / "css/glaze-v1.3.0-candidate.css").exists(), "Phase 0 must not create a Candidate CSS entrypoint")
    req(not (ROOT / "js/glaze-v1.3.0-candidate.mjs").exists(), "Phase 0 must not create a Candidate runtime entrypoint")
    ...
    print("GLAZE UI V1.3 Adaptive Resonance Phase 0: PASS")
    print("Boundary: planning is coherent; V1.2 remains Stable and V1.3 remains Proposed.")
    return 0

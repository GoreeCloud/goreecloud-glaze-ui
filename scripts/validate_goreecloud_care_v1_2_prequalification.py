...
def main() -> None:
    ...
    req(glaze.get("consumerAcceptanceState") == "prequalified-human-review-pending", "prequalification state")
    ...
    req(drift.get("blocksFinalConsumerAcceptanceWithoutExplicitResolution") is True, "stale prose must remain visible before final acceptance")

    pending = record.get("pendingHumanNativeGates")
    req(isinstance(pending, dict) and pending, "pending human/native gates")
    req(all(value is False for value in pending.values()), "no human/native gate may be fabricated as passed")
    ...
    req(decision.get("acceptedV1Allowed") is False, "accepted-v1 must remain blocked")
    req(decision.get("registryStatusMustRemain") == "adoption-required", "registry must remain adoption-required")
    req(decision.get("productionEligible") is False, "Glaze must not grant product production eligibility")
    req(decision.get("stableProductPromotionAllowed") is False, "Stable promotion must remain blocked")
    ...
    care = care[0]
    req(care.get("status") == "adoption-required", "Care registry status must remain adoption-required")
    ...
    print(
        ...
        "accepted-v1, production eligibility and Stable promotion remain blocked on human/native acceptance"
    )

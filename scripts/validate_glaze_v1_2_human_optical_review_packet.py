...
def validate_contract() -> tuple[dict[str, Any], list[Path]]:
    ...
    require(contract.get("exactHeadBinding") == {
        "source": "git rev-parse HEAD",
        "sameRevisionRequiredAcrossArtifacts": True,
        "staticRevisionInContractProhibited": True,
    }, "exact-head review binding drifted")
    require(re.search(r"\b[0-9a-f]{40}\b", CONTRACT.read_text(encoding="utf-8")) is None, "review-packet contract must not embed a static Candidate revision")

    evidence = contract.get("evidenceClasses")
    require(isinstance(evidence, list), "evidenceClasses must be an array")
    require([item.get("id") for item in evidence if isinstance(item, dict)] == EXPECTED_EVIDENCE_IDS, "review evidence class order/set drifted")
    ...
    for item in evidence:
        ...
        require(item.get("artifactNamePrefix") == artifact_prefix, f"{evidence_id}: artifact prefix missing")
        require(item.get("required") is True, f"{evidence_id}: review evidence became optional")
        workflow_paths.append(ROOT / workflow_path)

    require(contract.get("humanReview") == {
        "required": True, "status": "pending", "finalAuthority": True,
        "acceptedRevision": None, "reviewedAt": None, "reviewer": None,
...
def validate_authority_boundaries() -> None:
    ...
    require(visual.get("comparison", {}).get("humanOpticalReviewRemainsAuthoritative") is True, "human optical authority drifted")

    wall = load_json(ICON_WALL)
    require(wall.get("humanReview") == {
        "required": True, "status": "pending", "finalAuthority": True,
        "acceptedRevision": None, "reviewedAt": None,
    ...
    require(scenes.get("phase5ReferenceScenesComplete") is False and scenes.get("humanReviewPending") is True, "Reference Scene human acceptance unexpectedly completed")
    require(scenes.get("openSceneIds") == [], "bounded Reference Scene source set still open")
...
def validate_workflows(contract: dict[str, Any]) -> None:
    for item in contract["evidenceClasses"]:
        ...
        require("github.event.pull_request.head.sha || github.sha" in text, f"{evidence_id}: exact Candidate revision expression missing")
    ...
    living_text = (ROOT / EXPECTED_WORKFLOWS["living-frosted"][1]).read_text(encoding="utf-8")
...
def build_manifest(contract: dict[str, Any], sources: list[Path]) -> dict[str, Any]:
    ...
    evidence = [
        {
            ...
            "artifactNamePrefix": item["artifactNamePrefix"], "kind": item["kind"],
            "required": True, "requiredRevision": revision,
    ...
    return {
        ...
        "product": "GLAZE UI", "version": "1.2.0-candidate",
        "lifecycle": "candidate", "stableBaseline": "1.1.0",
        "revision": revision, "reviewStatus": "pending",
        "humanReviewRequired": True, "humanReviewComplete": False,
        "promotionEligible": False, "consumerEligible": False,
